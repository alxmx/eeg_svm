...
class RobustDataProcessor:
    """Handles real-time signal processing with robust outlier detection and filtering"""
    
    def __init__(self, window_size=250, outlier_threshold=3, median_filter_size=3):
        """Initialize with configurable parameters for signal processing"""
    ...
    def update_baseline_stats(self, eeg_data, eda_data):
        ...
        if len(eda_data) > 0:
            self.baseline_stats['eda'] = {
                ...
                'min': np.min(eda_data, axis=0),
                'max': np.max(eda_data, axis=0)
            }
    
    def process_eeg_window(self, eeg_window):
        """Apply robust processing to an EEG window"""
        if len(eeg_window) == 0:
            return np.zeros((self.window_size, 8))  # Empty window case
            
        # Ensure consistent shape
        if len(eeg_window.shape) == 1:
            eeg_window = eeg_window.reshape(1, -1)
            
        # Outlier detection and replacement
        if self.baseline_stats['eeg']:
            mean = self.baseline_stats['eeg']['mean']
            std = self.baseline_stats['eeg']['std']
            mask = np.abs(eeg_window - mean) > self.outlier_threshold * std
            eeg_window[mask] = mean[mask.reshape(-1)]
            
        # Apply median filter to each channel
        processed_window = eeg_window.copy()
    ...
    def process_eda_window(self, eda_window):
        """Apply robust processing to an EDA window"""
        if len(eda_window) == 0:
            return np.zeros((self.window_size, 2))  # Empty window case
            
        # Ensure consistent shape
        if len(eda_window.shape) == 1:
            eda_window = eda_window.reshape(1, -1)
            
        # Outlier detection and replacement
        if self.baseline_stats['eda']:
            mean = self.baseline_stats['eda']['mean']
            std = self.baseline_stats['eda']['std']
            mask = np.abs(eda_window - mean) > self.outlier_threshold * std
            eda_window[mask] = mean[mask.reshape(-1)]
            
        # Apply median filter to each channel
        processed_window = eda_window.copy()
        for ch in range(processed_window.shape[1]):
            processed_window[:, ch] = self.median_filter_1d(
                processed_window[:, ch], self.median_filter_size
            )
    ...
    def median_filter_1d(self, data, kernel_size=3):
        ...
        for i in range(radius, len(data) - radius):
            filtered[i] = np.median(data[i - radius:i + radius + 1])
    ...
    def normalize_eda_robust(self, raw_eda):
        ...
        if hasattr(self, 'adaptive_thresholds') and self.adaptive_thresholds:
            ...
            if 'mean' in eda_stats and 'std' in eda_stats:
                ...
                q95 = min(10, mean_eda + 2*std_eda)
                eda_min, eda_max = q5, q95
            else:
                # Default values if no calibration data
                eda_min, eda_max = 2.0, 12.0
        else:
            # Default values if no adaptive thresholds
            eda_min, eda_max = 2.0, 12.0
        
        # Apply robust clipping and normalization
        eda_clipped = np.clip(raw_eda, eda_min * 0.5, eda_max * 1.5)
        eda_norm = 10 * (eda_clipped - eda_min) / max(eda_max - eda_min, 0.1)  # Avoid division by zero
